...
| 后台驻留 | **关掉就是关掉**，不开机自启、无守护进程 | 三个常驻服务互相拉起 |
...
磁盘占用画成彩色拼图：大块头一眼可见，点击色块层层深入。色彩即语义——🔵 系统 / 🟣 软件 / 🟠 缓存 / 🟢 个人文件，配合 **60+ 条中文软件生态知识库**（剪映、微信、百度网盘、npm、conda……），每个目录都用人话告诉你"这是什么、能不能动"。扫描结果本地留存，下次打开秒显上次结果。
